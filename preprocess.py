...
def build_vocab(
    ...
    vocab_src, vocab_tgt = [
        ...
        for i, tokenize in enumerate(
            (get_tokenizer(spacy_src), get_tokenizer(spacy_tgt))
        )
    ]
...
def get_loader(
    iter_data: IterableDataset, batch_size: int, collate_fn, is_distributed=True
):
    """
...
def batch_size_fn(new_batch: Batch, count: int, so_far: int):
    ...
    max_tgt_in_batch = max(
        max_tgt_in_batch, len(new_batch.tgt) + 2
    )  # since we add a meta token at the beginning
...
def transform_text(
    ...
    wrapped = torch.cat(
        [bos_token, vocab_tokens, eos_token]
    )  # wrap with beginning and end of sentence
...
def collate_batch(
    ...
    def closure(text, tokenize, vocab):
        return transform_text(
            text, tokenize, vocab, device, n_ctx, bos_token, eos_token, pad_token
        )
